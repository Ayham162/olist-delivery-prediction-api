...
from inference_service.predict import get_model, get_model_source, get_run_config, model_version, predict_batch
...
async def lifespan(app: FastAPI):
    ...
    get_model()
    logger.info("service started | model_source=%s | model_version=%s", get_model_source(), model_version())
    yield


app = FastAPI(
    title="Olist Late-Delivery Inference Service",
    description="Predicts whether a new order will arrive after its estimated delivery date, with a probability.",
    lifespan=lifespan,
)
...
def predict(order: OrderInput):
    return predict_batch([order.model_dump()])[0]


@app.post("/predict/batch", response_model=BatchPredictionResponse)
def predict_batch_route(request: BatchPredictionRequest):
    results = predict_batch([o.model_dump() for o in request.orders])

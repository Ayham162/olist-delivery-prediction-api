...
# Building this image hit real ReadTimeoutErrors against
# files.pythonhosted.org, not a hypothetical concern.
RUN pip install --no-cache-dir --timeout 120 -r requirements.txt

# Editable install (-e) on purpose, even in this "final" image: config.py
...
# than getting physically copied into site-packages. A plain, non-editable
# `pip install .` did exactly that and broke config/model/data path
# resolution at container startup.
COPY pyproject.toml .
COPY src/ src/
...
# models/ and data/ are DVC-tracked, not committed to git. A genuinely fresh
# `git clone` doesn't have them, so `COPY models/ models/` from the host
# build context would fail on any machine but this one, where they already
# happen to exist locally from an earlier `dvc pull`/copy. The local DVC
# remote (../dvc-storage) is a path on this machine only, same problem CI
# hit (see .github/workflows/ci.yml). Fetched from the same GitHub Release
# asset here instead, so the image is self-sufficient on a truly clean
# clone with nothing more than `docker compose up --build`.
